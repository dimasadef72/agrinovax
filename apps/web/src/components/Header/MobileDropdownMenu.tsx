...
const MobileDropdownMenu = ({ isLoggedIn, onMenuClick }: Props) => (
  <div className="md:hidden mt-4 mb-4 flex flex-col items-center gap-4 text-[18px] text-[#2B3631] font-sans animate-slide-down">
    <Link href="/" className="hover:text-gray-500/75" onClick={onMenuClick}>
      Home
    </Link>
    <Link href="/maps" className="hover:text-gray-500/75" onClick={onMenuClick}>
      Maps
    </Link>
    <Link
      href="/contact"
      className="hover:text-gray-500/75"
      onClick={onMenuClick}
    >
      Contact
    </Link>
